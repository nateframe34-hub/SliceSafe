# SliceSafe - Changed Theme Files

Only **1 file** was modified for page speed optimization. Copy and paste the full contents below into your Shopify theme editor.
...
**What to do:** Select all existing content, delete it, paste everything below, then click Save.
...
<html class="no-js" lang="{{ request.locale.iso_code }}">
  <head>
    ...
    {%- comment -%}Move JS below CSS to avoid blocking render. Preconnect to fonts first.{%- endcomment -%}
    ...
    {% style %}
      ...
      {% endif %}
{% comment %}Removed main{visibility:hidden} - it blocks FCP by hiding content until JS runs{% endcomment %}
      ...
      *::after {
      ...
      /* Material Symbols loaded non-blocking below */
    ...
    {%- comment -%}Material Symbols loaded asynchronously to avoid blocking FCP{%- endcomment -%}
    <link rel="preload" href="https://fonts.gstatic.com/s/materialsymbolsoutlined/v141/kJF4BvYX7BgnkSrUwT8OhrdQw4oELdPIeeII9v6oDMzBwG-RpA6RzaxHMPdY40KH8nGzv3fzfVJU22ZZLsYEpzC_1qmr5Y0.woff2" as="font" type="font/woff2" crossorigin>
    <style>
      @font-face {
  ...
  <body class="gradient link-btns--{{ settings.link_btn_hover }} action-btns--{{ settings.action_btn_hover }}"{% if is_rtl %} data-rtl="true"{% endif %}>
    ...
    <script>
      {%- comment -%}Defer non-critical DOM work to after page is interactive{%- endcomment -%}
...
1. **Preconnect hints** moved before JS to establish font connections earlier
2. **Removed `main{visibility:hidden}`** that was blocking First Contentful Paint
3. **Material Symbols font** loaded via preload + inline @font-face with `font-display: swap` instead of render-blocking Google Fonts stylesheet
4. **All inline scripts** consolidated into one `DOMContentLoaded` handler with external link modifier moved into `requestIdleCallback`
